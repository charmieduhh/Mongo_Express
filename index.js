...
app.get('/farms', async (req, res) => { //farms index
...
app.get('/farms/:id', async (req, res) => { //async bc we need to find farm in db w/ await
    const farm = await Farm.findById(req.params.id);
    res.render('farms/show', { farm })
})

app.post('/farms', async (req, res) => { //making a new mode/saving it = async
...
app.get('/products', async (req, res) => { //waits for data to come back from mongoose & then responds
    const { category } = req.query; //looks to sees if there's a category in req.query
    if (category) { //if u did find category in req.query,
...
app.post('/products', async (req, res) => { //route where button submits to under new.ejs
...
app.put('/products/:id', async (req, res) => {
    const { id } = req.params;
    const product = await Product.findByIdAndUpdate(id, req.body, { runValidators: true, new: true }) //finds product & updates it @ the same time
